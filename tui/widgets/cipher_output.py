...
class CipherOutput(Widget):
    ...
    def __init__(self, title: str = "Resultado", **kwargs) -> None:
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="cipher-output-actions"):
            yield Button("Copiar al portapapeles", id="btn-copy", variant="success")
            yield Button("Guardar en archivo…", id="btn-save-toggle")
        with Horizontal(id="save-path-row"):
            yield Input(placeholder="/ruta/al/archivo.gpg", id="save-path-input")
            yield Button("Guardar", id="btn-save-confirm", variant="primary")
    ...
    def _save_file(self) -> None:
        ...
        if not path_str:
            self.app.notify("Ingresá una ruta para guardar.", severity="warning")
            return
        try:
            Path(path_str).write_text(self._content)
            self.app.notify(f"Guardado en {path_str}", severity="information")
